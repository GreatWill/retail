...
import com.svjia.common.AvgoutSql;
 ...
 * @Date: 2018/9/5 10:37
 */
@Service("avgoutService")
public class AvgoutService {

    private Log logger = LogFactory.getLog(AvgoutService.class);
    ...
    public Map<String,String> Total() throws Exception{
        ...
        PreparedStatement ps = connection.prepareStatement(AvgoutSql.TOTAL_ALL);
    ...
    private PreparedStatement getIncrementPsByType(String type, Connection conn) throws SQLException {
        PreparedStatement ps = conn.prepareStatement(AvgoutSql.TOTAL_BY_TIME);
        String[] dateArgs = PeinUtil.createPreDate(type);
        ps.setString(1, dateArgs[0]);
        ps.setString(2, dateArgs[1]);
        ps.setString(3, dateArgs[0]);
        ps.setString(4, dateArgs[1]);
        return ps;
    }
    ...
    private  PreparedStatement getRatePsByType(String type, String rate, Connection conn) throws Exception{
        PreparedStatement ps = conn.prepareStatement(AvgoutSql.RATE_BY_TIME);
        ...
        ps.setString(3,preDate[0]);
        ps.setString(4,preDate[1]);
        ps.setString(5,dateRateArg[0]);
        ps.setString(6,dateRateArg[1]);
        ps.setString(7,dateRateArg[0]);
        ps.setString(8,dateRateArg[1]);
        return ps;
    }
